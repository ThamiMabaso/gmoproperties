...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('company.messages.create', $company) }}" class="px-4 py-2 bg-gmo-gold text-black font-semibold rounded-md hover:bg-opacity-90 transition-colors">
            New Message
        </a>
    </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($messages as $message)
                        <tr class="hover:bg-gray-50 {{ !$message->is_read && $message->recipient_id === auth()->id() ? 'bg-blue-50' : '' }}">
                            ...
                            <td class="px-6 py-4">
                                <a href="{{ route('company.messages.show', [$company, $message]) }}" class="text-sm text-gray-900 hover:text-gmo-gold">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('company.messages.show', [$company, $message]) }}" class="text-gmo-gold hover:text-gmo-gold-dark mr-4">View</a>
    ...
    @else
        <div class="bg-white rounded-lg shadow p-12 text-center">
            ...
            <a href="{{ route('company.messages.create', $company) }}" class="inline-block px-4 py-2 bg-gmo-gold text-black font-semibold rounded-md hover:bg-opacity-90 transition-colors">
