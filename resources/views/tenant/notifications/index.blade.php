...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Alerts for applications, contracts, invoices, maintenance, and announcements.</p>
        </div>
            ...
            @foreach ($notifications as $n)
                ...
                <li class="px-6 py-4 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 {{ $n->read_at ? '' : 'bg-blue-50/50' }}">
                    ...
                    <div class="flex flex-wrap items-center gap-2 shrink-0">
                        ...
                        @endif
                        @if(! $n->read_at)
                            <form method="post" action="{{ route('tenant.notifications.read', $n->id) }}">
                                @csrf
